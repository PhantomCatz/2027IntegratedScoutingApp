...
export function TeamLookup() {
  ...
  const [openTeams, setOpenTeams ]= useState<number[]>(teams?.split(",").map(Number).filter((n) => n > 0) ?? []);
  ...
  function syncUrl(next: number[]) {
    navigate(
      next.length ? `/teamlookup/${next.join(",")}` : "/teamlookup",
      { replace: true }
    );
  }
  ...
  return (
    ...
    >
      <fieldset className="fieldset rounded-box p-4">
        <legend className="fieldset-legend text-xl">Team Lookup</legend>
        <input
          type="number"
